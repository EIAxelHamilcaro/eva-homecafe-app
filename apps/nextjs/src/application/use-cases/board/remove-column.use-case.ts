...
import type {
  IRemoveColumnInputDto,
  IRemoveColumnOutputDto,
} from "@/application/dto/board/remove-column.dto";
import type { IBoardRepository } from "@/application/ports/board-repository.port";
import { BoardId } from "@/domain/board/board-id";

export class RemoveColumnUseCase
  implements UseCase<IRemoveColumnInputDto, IRemoveColumnOutputDto>
{
  constructor(private readonly boardRepo: IBoardRepository) {}

  async execute(
    input: IRemoveColumnInputDto,
  ): Promise<Result<IRemoveColumnOutputDto>> {
    ...
    const removeResult = board.removeColumn(input.columnId);
